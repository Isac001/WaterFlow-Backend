...
def bimonthly_water_consumption():
    try:
        ...
        # Calcula o primeiro mês do bimestre (1, 3, 5, 7, 9, 11)
        first_month = ((current_month - 1) // 2) * 2 + 1
        first_day = datetime(current_year, first_month, 1).date()

        # Define o último dia do segundo mês do bimestre
        if first_month == 11:
            last_day = datetime(current_year, 12, 31).date()
        else:
            # Para os outros bimestres: pega o 1º dia do mês após o bimestre e subtrai 1 dia
            last_day = (datetime(current_year, first_month + 2, 1).date() - timedelta(days=1))

        # Busca registros mensais que intersectam o bimestre
        records = MonthlyWaterConsumption.objects.filter(
        ...
        # Verifica se já existe registro bimestral para esse período
        if BimonthlyWaterConsumption.objects.filter(
            start_date=first_day,
            end_date=last_day
        ).exists():
