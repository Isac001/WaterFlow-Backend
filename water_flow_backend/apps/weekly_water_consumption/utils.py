...
# Import the FlowRating model (though not directly used in this function, it's imported)
...
def calculate_weekly_water_consumption():
    ...
    try: 
        ...
        if WeeklyWaterConsumption.objects.filter(
            # Filter by the exact start date of the week
            start_date=start_day,
            # Filter by the exact end date of the week
            end_date=end_day
        ...
        return WeeklyWaterConsumption.objects.create(
            # Set the descriptive label for the weekly consumption period
            date_label=f"Consumo de água da semana de {start_day.strftime('%d/%m/%Y')} a {end_day.strftime('%d/%m/%Y')}",
            # Set the start date of the weekly record
            start_date=start_day,
            # Set the end date of the weekly record
            end_date=end_day,
