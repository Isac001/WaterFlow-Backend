...
# Import the DailyWaterConsumption model for creating test data
from apps.daily_water_consumption.models import DailyWaterConsumption
# Import the Celery task for weekly water consumption calculation
        ...
        try:
            ...
            for days_ago in range(7):
                ...
                DailyWaterConsumption.objects.create(
                    # Set the date label in the format "Total de litros consumidos do dia DD de Month de YYYY"
                    date_label=f"Total de litros consumidos do dia {current_date.strftime('%d de %B de %Y')}", # Assumes locale is Portuguese for %B
                    # Set the total consumption for the day
